...
func (p *OSTreeDeployment) serialize() (osbuild.Pipeline, error) {
	...
	// Set up default mounts for the pipeline. These are used by all stages in this pipeline except those
	// who explicitly opt out of them.
	pipeline.SetDefaultMounts(*osbuild.NewOSTreeDeploymentMount("ostree-"+ref, p.osName, ref, 0))
	...
	if p.IgnitionPlatform != "" {
		...
		// bypasses the pipeline default mounts.
		pipeline.Stages = append(pipeline.Stages, osbuild.NewIgnitionStage(&osbuild.IgnitionStageOptions{
	...
	// not through mounts, so it bypasses the pipeline default mounts.
	pipeline.Stages = append(pipeline.Stages, osbuild.NewOSTreeSelinuxStage(
